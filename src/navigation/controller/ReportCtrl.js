mNavigation.controller('ReportCtrl',
    function($rootScope, $scope, $http, $filter, $timeout,
        ...
        //         $timeout(function() {
        //             console.log("finished getting order-data");
        //                 $scope.$apply(function(){
        //                 // $scope.isGettingData = false;
        //                 // console.log(response);
        //                 $scope.data = response;
        ...
        $rootScope.downloadCSV = function() {
            ...
            var res = [];
            angular.forEach($scope.data.data, function(order){
                if(order.customer_mobile){
                    // console.log('mobile: ', order.customer_mobile);
                    res.push({
                        mobile: "+84" + order.customer_mobile.substring(1, order.customer_mobile.length),
                        page: order.page_id,
                    })
                   }
        ...
        // console.log("getting all shippings");
        // $http.get('../assets/meo4-f0335-shippingItems-export.json').
        //       then(function onSuccess(response) {
        //         $timeout(function() {
        //             console.log("finished getting all shippings");
        //                 $scope.$apply(function(){
        //                 // $scope.isGettingData = false;
        //                 // console.log(response);
        ...
        $rootScope.downloadShippingsCSV = function() {
            if(!$scope.shippings.data || $scope.shippings.data.length == 0){
                MUtilitiesService.AlertError('Không có dữ liệu', 'Lỗi');
                return null;
            }
            var res = [];
            angular.forEach($scope.shippings.data, function(order){
                if(order.customer_mobile){
                    ...
                    res.push({
                        mobile: "+84" + order.customer_mobile.substring(1, order.customer_mobile.length),
                        page: order.data.orderData.page_id,
                        product1: product1 ? product1.id : '',
                        product2: product2 ? product2.id : '',
                        product2: product2 ? product2.id : '',
                    })
                   }
